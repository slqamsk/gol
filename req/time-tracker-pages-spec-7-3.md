...
- В текущем прототипе поле «Перерывы из-за прерываний» (interruptBreaks) доступно для ручного редактирования. Это временное решение для тестирования. В финальной версии поле будет удалено из интерфейса, а его значение будет вычисляться автоматически на основе пересечений типов interrupt overlap. В логике валидации (раздел 7.3) пока оставляем возможность редактирования, но в диалогах Случая 2, шаг 2, не добавляем вариант изменения interruptBreaks, так как в будущем оно станет вычисляемым.
